...
import {
  ngOneSignalAsyncFunctionTemplate,
  ngOneSignalFunctionTemplate,
} from '../../support/angular/oneSignalFunctionTemplates';
...
export abstract class OneSignalWriterManagerBase extends CodeWriter {
  ...
  protected async writeNamespaceExport(
    api: IOneSignalApi,
    ...
    if (properties) {
      properties.forEach((prop) => {
        ...
        this.writeLine(
          `\tget ${prop.name}(): ${prop.type} { return window.OneSignal?.${PARENT_PATH_FOR_NAMESPACE_MAP[namespaceName] ?? ''}${namespaceName}?.${prop.name}${defaultValue}; },`,
        );
      });
    }
    ...
    functions.forEach((func) => {
      ...
      this.writeLine(
        `${prefix}${func.name}: ${generateUniqueFunctionName(namespaceName, func.name)},`,
      );
    });
